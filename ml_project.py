...
class MLProject:
    ...
    def _grade_ml_section(self, button_x, text_x, save_x, msg, save_x2_li=list(), section=None):
        ...
        try:
            self.grader.find_by_xpath_click(button_x)
            e = self.grader.browser.find_element(By.XPATH, text_x)
            self.grader.scroll_into_view(e)
            e.send_keys(msg)
            self.grader.find_by_xpath_click(save_x)

        # if we're here, then this section has likely already been graded
        except NoSuchElementException:
            for x in save_x2_li:
                try:
                    self.grader.find_by_xpath_click(x)
                    return
                except NoSuchElementException:
                    pass
    ...
    def _grade_arbitrary_ml_section(self):
        ...
        for p in self.perms:
            ...
            if xpath not in self.known_xpaths:
                try:
                    self.grader.find_by_xpath_click(xpath)
                    return p
                except NoSuchElementException:
                    pass
    ...
    def _grade_all_ml_sections(self):
        ...
        self._grade_ml_section(_button_x1, _text_x1, _save_x1, msg_1, x1, 1)
        self._grade_ml_section(_button_x2, _text_x2, _save_x2, msg_2, x2, 2)
        self._grade_ml_section(_button_x3, _text_x3, _save_x3, msg_3, x3, 3)
        self._grade_ml_section(_button_x4, _text_x4, _save_x4, msg_4, x4, 4)
        self._grade_ml_section(_button_x5, _text_x5, _save_x5, msg_5, x5, 5)
        self._grade_ml_section(_button_x6, _text_x6, _save_x6, msg_6, x6, 6)
        self._grade_ml_section(_button_x7, _text_x7, _save_x7, msg_7, x7, 7)
        self._grade_ml_section(_button_x8, _text_x8, _save_x8, msg_8, x8, 8)
        self._grade_ml_section(_button_x9, _text_x9, _save_x9, msg_9, x9, 9)
        self._grade_ml_section(_button_x10, _text_x10, _save_x10, msg_10, x10, 10)
        self._grade_ml_section(_button_x11, _text_x11, _save_x11, msg_11, x11, 11)
        self._grade_ml_section(_button_x12, _text_x12, _save_x12, msg_12, x12, 12)
        self._grade_ml_section(_button_x13, _text_x13, _save_x13, msg_13, x13, 13)
    ...
    def _fill_final_section_ml(self):
        ...
        e = self.grader.browser.find_element(By.XPATH, final_text_xpath)
        self.grader.scroll_into_view(e)
        e.send_keys(msg)
        self.grader.browser.find_element(By.XPATH, final_save_button_xpath).click()
